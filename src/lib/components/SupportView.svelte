<script lang="ts">
	...
	let ownerName = $derived(overview?.ownerName.trim() ?? '');
	let ownerLabel = $derived(ownerName || 'the account owner');
	...
	<section class="standard-page support-page" aria-labelledby="support-title">
		<div class="support-heading">
			<div>
				...
				<h1 id="support-title">Family support</h1>
				<p>A simple, privacy-limited view for trusted people.</p>
			</div>
			<div class="support-heading-actions"><button class="secondary-button" aria-label="Manage access" onclick={onManage}><Icon name="shield" size={20} /> Manage</button><button class="secondary-button" aria-label={ownerName ? `Open ${ownerName}'s board` : 'Open main board'} onclick={onOpenBoard}><Icon name="calendar" size={20} /> Board</button></div>
		</div>
		...
		<div class="support-metrics" aria-label="At a glance">
			<article>
				<span class="metric-icon care"><Icon name="care-visit" size={25} /></span>
			...
			<article>
				...
				<div><small>{ownerName ? `For ${ownerName}` : 'Private items'}</small>{#if overview.attentionCount === undefined}<strong>Private</strong><span>Review counts are not shared</span>{:else}<strong>{overview.attentionCount} to review</strong><span>Only {ownerLabel} can decide</span>{/if}</div>
			</article>
		</div>
		...
		{#if overview.helpRequests.length}
			<section class="help-request-banner" aria-labelledby="help-request-title">
				...
				<div><span class="eyebrow">Asked for help</span><h2 id="help-request-title">{overview.helpRequests[0].label}</h2><p>{overview.helpRequests[0].status === 'help_requested' ? ownerName ? `${ownerName} is waiting for someone to respond.` : 'A response is still needed.' : 'You said you are helping with this.'}</p></div>
		...
		<div class="support-layout">
				...
				<div class="support-schedule">
					{#each overview.schedule as item}
						<article>
							...
							<span class="mini-icon {item.kind}"><Icon name={item.kind === 'care' ? 'care-visit' : item.kind === 'food' ? 'basket' : item.kind === 'health' ? 'heart' : 'walk'} size={20} /></span>
			...
			<section class="support-card" aria-labelledby="offer-help-title">
				...
				<p class="support-intro">Your offer appears on the main board. It does not change the calendar until it is accepted.</p>
				<div class="support-actions">
					...
					{/if}
					<article>
						<span class="action-icon"><Icon name="phone" size={23} /></span>
				...
				{#each overview.acceptedAssignments as assignment}
					<div class="accepted-assignment">
						<div><strong>{assignment.status === 'completed' ? 'Help completed' : assignment.status === 'acknowledged' ? 'You are helping' : ownerName ? `${ownerName} accepted your help` : 'Your help was accepted'}</strong><span>{assignment.message}</span></div>
